"""Batch unshadow endpoint scaffolding."""
...
@router.post("/run/batch_unshadow", response_model=JobHandle, status_code=status.HTTP_202_ACCEPTED)
async def run_batch_unshadow(
    payload: BatchUnshadowRequest,
    auth_user: AuthenticatedUser = Depends(get_current_user),
    executor: AdminToolsExecutor = Depends(get_admin_tools_executor),
) -> JobHandle:
    try:
        return await executor.run_batch_unshadow(payload, actor_id=auth_user.id)
