...
export default function LeaderboardsPage() {
  ...
  // Fetch Leaderboard - use "overall" scope which shows Total XP
  useEffect(() => {
    if (!authEvaluated || !campusId) return;
    const controller = new AbortController();
    setLoading(true);
    setError(null);
    fetchLeaderboard("overall", { campusId, signal: controller.signal })
      .then(setLeaderboard)
      .catch((err) => {
  ...
  return (
    <main className="min-h-screen w-full bg-slate-50 px-4 py-8">
      <div className="mx-auto flex max-w-4xl flex-col gap-8">
        ...
        {/* Total XP Header */}
        <div className="flex items-center justify-center gap-4 rounded-2xl bg-white p-6 border border-slate-200 shadow-sm">
          <div className="flex h-14 w-14 items-center justify-center rounded-2xl bg-amber-100 text-amber-600">
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="h-7 w-7">
              <path strokeLinecap="round" strokeLinejoin="round" d="M16.5 18.75h-9m9 0a3 3 0 013 3h-15a3 3 0 013-3m9 0v-3.375c0-.621-.503-1.125-1.125-1.125h-.871M7.5 18.75v-3.375c0-.621.504-1.125 1.125-1.125h.872m5.007 0H9.497m5.007 0V5.625a1.125 1.125 0 00-1.125-1.125H10.625a1.125 1.125 0 00-1.125 1.125v8.625" />
            </svg>
          </div>
          <div className="text-left">
            <h2 className="text-2xl font-bold text-slate-900">Total XP</h2>
            <p className="text-sm text-slate-600">Your total experience points earned from all campus activities, games, and social interactions.</p>
          </div>
        </div>
