...
import { useGroupEvents } from "@/hooks/communities/use-events";
...
export function GroupEventsBoard({ groupId }: { groupId: string }) {
  const [scope, setScope] = useState<EventsScope>("upcoming");
  const [view, setView] = useState<EventsView>("list");

  const query = useGroupEvents(groupId, scope);
