...
export default function CoursesOnboardingPage() {
  ...
  const handleSubmit = async () => {
    ...
    setSubmitting(true);
    try {
      const courseInputs = selectedCourses.map((code) => {
        const popular = popularCourses.find((c) => c.code === code);
        return {
          name: popular?.name || code,
          code: code,
          term: "",
        };
      });
      await saveProfileCourses(userId, campusId, courseInputs);
      setSubmitting(false);
      // Show welcome screen
      setWelcome(true);
      setTimeout(() => {
  ...
  return (
    <main className="min-h-screen w-full bg-white text-base">
      <div className="mx-auto flex min-h-screen max-w-7xl flex-col gap-10 px-6 py-12 lg:flex-row lg:items-center lg:gap-8">
        ...
        <section className="flex flex-1 justify-center">
          <div className="w-full max-w-xl rounded-3xl bg-white px-6 py-8 shadow-2xl ring-1 ring-[#f0d8d9]/80 sm:px-9">
              ...
              <div className="flex flex-wrap gap-3">
                {popularCourses.map((course) => (
                  <button
                    ...
                    className={`rounded-full px-4 py-2 text-sm font-medium transition ${selectedCourses.includes(course.code)
            ...
            <div className="mt-auto flex justify-end">
              <button
                ...
                disabled={submitting}
                className="w-full rounded-xl bg-[#d64045] px-8 py-4 text-lg font-semibold text-white shadow-md transition hover:bg-[#c7343a] disabled:opacity-60"
              >
