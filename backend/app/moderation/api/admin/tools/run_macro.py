"""Macro execution endpoint scaffolding."""
...
@router.post("/run/macro", response_model=JobHandle, status_code=status.HTTP_202_ACCEPTED)
async def run_macro(
    ...
    try:
        return await executor.run_macro(payload, actor_id=auth_user.id)
