...
export default function ActivitiesHub() {
  ...
  const { invite: storyInvite } = useStoryInvite();
  ...
  const hasPendingStoryInvite = useMemo(() => Boolean(storyInvite), [storyInvite]);
  ...
  return (
    <main className="relative min-h-screen overflow-hidden bg-[#0c0b16] text-white">
      ...
      {/* Removed background logo overlay to match page background */}
      <div className="relative mx-auto max-w-7xl px-6 py-12">
        <header className="max-w-3xl space-y-3">
          ...
          <p className="text-sm text-rose-100/80">
            Every activity now opens from a window-style preview with your familiar Campus backdrop. Click any window to launch the experience instantly.
          </p>
        </header>
        ...
        <section className="mt-10 grid gap-6 md:grid-cols-2 xl:grid-cols-3">
          {CATALOG.map((item) => {
            const highlight =
              (item.key === "speed_typing" && hasPendingSpeedTypingInvite) ||
              (item.key === "quick_trivia" && hasPendingQuickTriviaInvite) ||
              (item.key === "rps" && hasPendingRpsInvite) ||
              (item.key === "story" && hasPendingStoryInvite);
            const shouldFocusInviteView = highlight && ["speed_typing", "story"].includes(item.key);
            const targetHref = shouldFocusInviteView && item.href ? `${item.href}?focus=invites` : item.href;
            const content = (
              <article
                key={item.key}
              ...
              >
                <div className="flex items-center gap-2 border-b border-white/10 bg-white/5 px-4 py-3">
                  ...
                  {!item.available && (
                    <span className="ml-2 rounded-full bg-white/10 px-2 py-0.5 text-[10px] font-semibold text-white/70">Coming soon</span>
                  )}
                </div>

                <div className="relative h-52 overflow-hidden bg-gradient-to-br from-rose-200/20 via-indigo-500/10 to-slate-900/30">
                  {item.img ? <Image src={item.img} alt="" fill className="object-cover object-center opacity-80" /> : null}
                ...
                <div className="flex flex-col gap-3 px-4 py-4">
                  ...
                  <div className="flex items-center gap-2 text-[11px] text-rose-50/80">
                    ...
                    <span className="ml-auto rounded-full bg-white/10 px-2 py-0.5 text-[10px] font-semibold text-white/80 ring-1 ring-white/15">2 players</span>
                  </div>
                  <div className="pt-1">
                    <span
                      className={`inline-flex w-full items-center justify-center rounded-xl px-4 py-2 text-sm font-semibold transition ${item.available && item.href
                        ? "bg-rose-500 text-white shadow-lg shadow-rose-500/30 hover:bg-rose-400"
                        : "cursor-not-allowed bg-white/10 text-white/50"}`}
                    >
                      {item.available && item.href ? (highlight ? "View invite" : "Open activity window") : "Coming soon"}
                    </span>
                  </div>
                </div>
              </article>
            ...
            return (
              <Link key={item.key} href={targetHref!} className="focus:outline-none focus:ring-2 focus:ring-rose-200 focus:ring-offset-2 focus:ring-offset-[#0c0b16]">
                {content}
              </Link>
            );
          })}
