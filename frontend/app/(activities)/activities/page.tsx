    ...
    <main className="mx-auto max-w-6xl px-6 py-10">
      <header className="space-y-2">
        ...
        <p className="max-w-3xl text-sm text-navy/70">
          ...
          friend, or explore what’s coming next.
        </p>
      </header>

      <section className="mt-6 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
        {CATALOG.map((item) => (
          <article key={item.key} className="group relative overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm transition hover:-translate-y-0.5 hover:shadow-md">
            ...
            <div className="flex flex-col p-4">
              ...
              <div className="mt-4">
                {item.available && item.href ? (
                  <Link
                    href={item.href}
                    className="inline-flex w-full items-center justify-center rounded-full bg-sky-600 px-4 py-2 text-sm font-semibold text-white shadow hover:bg-sky-500 focus:outline-none focus:ring-2 focus:ring-sky-300"
                  >
                    Start with a friend →
                  </Link>
                ) : (
