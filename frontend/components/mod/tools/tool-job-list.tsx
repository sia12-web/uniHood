...
export function ToolJobList({ jobs, loading = false, selectedJobId, onSelect }: ToolJobListProps) {
	return (
		<section className="space-y-3">
			...
			<ul className="divide-y divide-slate-200 overflow-hidden rounded-2xl border border-slate-200 bg-white">
				{jobs.length === 0 ? (
					<li className="px-4 py-6 text-sm text-slate-500">No jobs yet. Trigger a tool run to populate the list.</li>
				) : (
					jobs.map((job) => {
						const isSelected = job.id === selectedJobId;
						const progressText = job.total != null ? `${job.succeeded ?? 0}/${job.total}` : job.succeeded ?? "—";
						return (
							<li key={job.id}>
								<button
									type="button"
